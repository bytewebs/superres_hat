...
if __name__ == '__main__':
    ...
    if args.target_adapt:
        cmd = f'python eval_glean_all.py --checkpoint_g {args.checkpoint_g} --checkpoint_d {args.checkpoint_d} --do_TA --batch {args.batch} --nepoch {args.nepoch} --data_path {args.data_path}'
    else:
        cmd = f'python eval_glean_all.py --checkpoint_g {args.checkpoint_g} --checkpoint_d {args.checkpoint_d} --batch {args.batch} --nepoch {args.nepoch} --data_path {args.data_path}'
    print(cmd)
    execute(cmd)
    ...
    out_fn = f'{args.data_path[:-4]}_senglean_enhanced.tif'
    generate_raster_senglean(out_fn, chpt, raster_file=args.data_path, scale=2, mscale=2, pad=5, model='senglean')
