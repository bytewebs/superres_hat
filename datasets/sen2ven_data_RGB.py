...
class TrainData(Dataset):
    def __init__(self, data_path, s):
        ...
        self.venus_data2 = glob.glob(f'{data_path}/*05m*8.pt')  # MuS2
        self.max_value = 2 ** s.nbits
        self.s = s

    def __getitem__(self, index):
        if index < len(self.venus_data):
            if np.random.uniform() < 0.9:
                venus_fl = self.venus_data[index]
            else:
                venus_fl = self.venus_data2[index]
        else:
            venus_fl = self.venus_data2[index]
        ...
        data_type = random.choices(['sen2ven', 'sen', 'ven'], [0.4, 0.2, 0.4])[0]
        ...
        return sentinel[:3].float(), venus[:3].float(), venus_up[:3].float(), sentinel_up[:3].float()

    def __len__(self):
        return len(self.venus_data2)
