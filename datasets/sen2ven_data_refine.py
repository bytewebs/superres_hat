...
from utils import random_crop_torch, random_flip2
from datasets.img_process_utils import deform_single_refine
...
class TrainData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        venus = torch.cat((venus_rgb, venus))    # b2 b3 b4 b5 b6 b7 b8a  7 channels
        
        venus_degrade = torch.zeros_like(venus)
        for i in range(7):
            venus_degrade[i] = deform_single_refine(venus[i].unsqueeze(0), 1)

        # Random flip
        venus, venus_degrade = random_flip2(venus, venus_degrade)
        
        venus, venus_degrade = random_crop_torch(venus, venus_degrade, 256, 1)
        
        return venus_degrade[:].float(), venus[:].float()
