...
from losses.texture_loss import TextureLoss
from networks import RRDBNet, DiscriminatorSentinel, UNetDiscriminatorSN, NLayerDiscriminator
from utils import savemat_plot_senven_20mnorm, psnr, psnr_bands, rmse, rmse_bands, center_crop, sam_batch, uqi_batch, scc_batch
from datasets.sen2ven_data_20m4_norm import TrainData, ValidData
from datasets.img_process_utils import USMSharp
from datasets.niqe_iqa import NIQE
from utils import recover_bandwise_20m, recover_bandwise_RGB, on_load_checkpoint
...
def test_single(data, net_g, net_d, iters):
    ...
    metrics['RMSE'] = rmse(recover_bandwise_20m(ms_fake_crop), recover_bandwise_20m(ven_up_crop))    # Caution
    metrics_bi['RMSE'] = rmse(recover_bandwise_20m(ms_up_crop), recover_bandwise_20m(ven_up_crop))      # Caution
    
    rmse_bands_res = rmse_bands(recover_bandwise_20m(ms_fake_crop), recover_bandwise_20m(ven_up_crop))
    rmse_bands_res_bi = rmse_bands(recover_bandwise_20m(ms_up_crop), recover_bandwise_20m(ven_up_crop))
    ...
    # print(ms_fake_crop.flip(1)[:,1:].shape, ms_up_crop.flip(1)[:,1:].shape)
    ...
    savemat_plot_senven_20mnorm(ms_fake, ms_up, ven_up, save_dir, s, ms_rgb_up, filename=f'plot_{iters}.png')
...
def train(args, netG, netD):
    ...
    if (args.checkpoint != None)&(args.resume == True):
        ...
        checkpoint_dict = torch.load(args.checkpoint, map_location=torch.device(device))
        netG, _ = on_load_checkpoint(netG, checkpoint_dict, strict=True)
        
    elif (args.resume==True):
        print('Loading pre-defined default models')
        # Adversarial Training
        netG.load_state_dict(torch.load(f'{checkpoint_dir}gens3n4_14_229.pt', map_location=torch.device(device)), strict=False)
        # 0_399 {checkpoint_dir}
    
    params = torch.load(f'{checkpoint_dir}discs3n4_14_229.pt', map_location=torch.device(device))
    # params = torch.load(f'{checkpoint_dir}disc.pt', map_location=torch.device(device))
    ...
    optimizer_dict = torch.load(f'{checkpoint_dir}optimizer3n4_14_.pt', map_location=torch.device(device))

    optimizerG = optim.Adam(netG.parameters(), config.TRAIN.lr_adv)
    optimizerD = optim.Adam(netD.parameters(), config.TRAIN.lr_adv/2)
    ...
    for epoch in range(n_epoch):
        ...
        for i, data in enumerate(dataloader):
            ...
            if do_usm:
                ...
                content_loss_rgb = feature_criterion(ms_fake[:,:3].flip(1), ven_up_usm[:,:3].flip(1))[0].sum()*0.8
                content_loss_nir = feature_criterion(ms_fake[:,3:].repeat(1, 3, 1, 1), ven_up_usm[:,3:].repeat(1, 3, 1, 1))[0].sum()*0.2
            ...
            errG = (15*errG_spec) + (0.1*errG_adv) + (3*content_loss) + (5e2*errG_artifacts) + (errG_ssim*10) + (7*errG_tv) + (errG_ndvi*3)
            ...
            if i%10 == 0:
                ...
                if i%20==0:
                    ...
                    savemat_plot_senven_20mnorm(ms_fake, ms_up, ven_up, save_dir, s, d_gen, filename=f'plot_train_{iters}.png')
                else:
                    d_gen = torch.abs(ms_fake - ms_up).mean(1)
                    savemat_plot_senven_20mnorm(ms_fake, ms_up, ven_up, save_dir, s, d_gen, abs_diff=True, filename=f'plot_train_{iters}.png')
                ...
                if args.do_log:
                    ...
                    test_hist = wandb.Image(os.path.join(save_dir, f'plot_{iters}_hist.png'), caption=f"Histogram Iteration {iters}")
                    wandb.log({"errD_loss": errD.item(), 'errG_loss':errG.item(), 'Spectral loss':errG_spec.item(), 'TV loss':errG_tv.item(),
                            ...
                            "Plots":image, "Plots training":train_image, "Histogram testing":test_hist})
                
            
        if epoch%20 == 9:
            torch.save(netG.state_dict(), f'{checkpoint_dir}gens3n4_15_{epoch}.pt')
            torch.save(netD.state_dict(), f'{checkpoint_dir}discs3n4_15_{epoch}.pt')
            torch.save({'optimizerG': optimizerG.state_dict(),
                        'optimizerD': optimizerD.state_dict()},
                        f'{checkpoint_dir}optimizer3n4_15_.pt')
        schedulerG.step()
        schedulerD.step()
...
if __name__ == '__main__':
    ...
    save_dir = "../srgan_samples_sent_glean4/"
    checkpoint_dir = "../checkpoints_glean/"
    ...
    niqe = NIQE(s.ratio, './losses/niqe_model.mat').to(device)
