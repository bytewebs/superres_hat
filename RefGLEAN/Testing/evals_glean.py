...
from utils import savemat_plot_senven_20mnorm, psnr, psnr_bands, rmse, rmse_bands, sam_batch, uqi_batch, scc_batch, center_crop, cc_batch, d_lambda_batch, d_s_batch
from pytorch_msssim import SSIM
from datasets.niqe_iqa import NIQE
from utils import recover_bandwise_20m, recover_bandwise_RGB
from torchmetrics.image import ErrorRelativeGlobalDimensionlessSynthesis as ERGAS
...
def test(args):
    ...
    netG_rgb = nn.DataParallel(netG_rgb)
    netG_rgb.load_state_dict(torch.load(args.checkpoint_rgb, map_location=torch.device('cpu')))
    ...
    if args.checkpoint_g != None:
        netG.load_state_dict(torch.load(args.checkpoint_g, map_location=torch.device('cpu')))
    else:
        netG.load_state_dict(torch.load(config.TEST.checkpoint_path))

    # Reading Dataset
    if args.data_path is not None:
        from datasets.patch_data_20m_norm import TestData_single, TestData_TA_single
    ...
    if args.target_adapt:
        ...
        if args.data_path is not None:
            netD.load_state_dict(torch.load(args.checkpoint_d, map_location=torch.device('cpu')))
        else:
        ...
        eval_dataloader = DataLoader(test_ta, batch_size=1, shuffle=True, drop_last=True)
        ...
        schedulerG = optim.lr_scheduler.MultiStepLR(optimizerG, [100], 0.8)
        ...
        for epoch in range(n_epoch_init):
            ...
            for i, data in enumerate(ta_dataloader):
                ...
                if not args.interp:
                   ms_rgb = netG_rgb(ms_rgb)[:,:]
                else:
                    ms_rgb = F.interpolate(ms_rgb, scale_factor=2, mode="nearest")  # bicubic
                ...
                errG_ndvi = ndvi_loss((ms_fake[:,3]-ms_fake[:,2]), (ms_up[:,3]-ms_up[:,2]))
                ...
                errG_ssim = 1 - ssim_module(ms_fake, ms_up)
                if args.do_artif:
                ...
                gen_total_loss = (23*gen_spectral_loss) + (errG_tv*5) + (3*content_loss) + (0.1*adv_loss) + (5e2*errG_artifacts) + (errG_ssim*2) + (errG_ndvi*3)
                ...
                print(ms_fake.shape, ms.shape, 'fake_ms')
                ds_res = d_s_batch(ms_up[:,:].detach().cpu(), ms[:,:].detach().cpu(), ms_fake[:,:].detach().cpu())
                ds_res_bi = d_s_batch(ms_up[:,:].detach().cpu(), ms[:,:].detach().cpu(), ms_interp[:,:].detach().cpu())

                dlam_res = d_lambda_batch(recover_bandwise_20m(ms_up, norm_data=norm), recover_bandwise_20m(ms_fake, norm_data=norm))
                dlam_res_bi = d_lambda_batch(recover_bandwise_20m(ms_up, norm_data=norm), recover_bandwise_20m(ms_interp, norm_data=norm))
                ...
                niqe_res_at = niqe(ms_interp.flip(1)[:,1:]*255).mean().item()
                ...
                if iters%2 == 0:
                    ...
                    savemat_plot_senven_20mnorm(ms_fake, ms_interp, ms_up, save_dir, s, d_gen, abs_diff=True, filename=f'plot_{iters}.png')
                elif iters%2 == 1:
        ...
        torch.save(netG.state_dict(), f'{checkpoint_dir}gen_band_{args.model}_TA.pt')
        ...
        for i, data in enumerate(eval_dataloader):
            ...
            if not args.interp:
                ...
                ms_rgb = F.interpolate(ms_rgb, scale_factor=2, mode="nearest")
            ms_fake = netG(ms, ms_rgb)
            ...
            ms_interp = center_crop(ms_interp, int(H*0.9))
            ...
            rmse_bands_res = rmse_bands(recover_bandwise_RGB(ms_fake), recover_bandwise_RGB(ms_up))
            rmse_bands_res_bi = rmse_bands(recover_bandwise_RGB(ms_interp), recover_bandwise_RGB(ms_up))
            ...
            niqe_res = niqe(ms_fake.flip(1)[:,1:]*255).mean().item()
            niqe_res_bi = niqe(ms_interp.flip(1)[:,1:]*255).mean().item()
            niqe_res_at = niqe(ms_up.flip(1)[:,1:]*255).mean().item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_rgb', type=str, default=None, help='Select weights by providing path')
    ...
    parser.add_argument('--model_rgb', type=str, default='esrgan', help='Select model type')
    ...
    parser.add_argument('--artif', dest='do_artif', action='store_false')
