...
class AAN(nn.Module):
    ...
    def __init__(self, nchannels=7):
        ...
        ### first convolution
        self.conv_first = nn.Conv2d(nchannels, nf, 3, 1, 1, bias=True)
        
        ### main blocks
        self.AAB_trunk = make_layer(AAB_block_f, nb)
        self.trunk_conv = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        
        self.conv_last = nn.Conv2d(nf, nchannels, 3, 1, 1, bias=True)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
    ...
    def forward(self, x):
        ...
        trunk = self.trunk_conv(self.AAB_trunk(out))
        out = out + trunk
