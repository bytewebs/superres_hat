...
def train(args, netG, netD):
    ...
    if (args.checkpoint != None)&(args.resume == True):
        ...
        netG.load_state_dict(torch.load(f'{checkpoint_dir}gensli_49.pt', map_location=torch.device(device)), strict=False)
    
    params = torch.load(f'{checkpoint_dir}discsli_49.pt', map_location=torch.device(device))
    # params = torch.load(f'../checkpoints_glean/discsli_49.pt', map_location=torch.device(device))
    ...
    optimizer_dict = torch.load(f'{checkpoint_dir}optimizerli_.pt', map_location=torch.device(device))
    ...
    for epoch in range(n_epoch):
        ...
        for i, data in enumerate(dataloader):
            ...
            errG = (15*errG_spec) + (0.05*errG_adv) + (3*content_loss) + (5e2*errG_artifacts) + (errG_ssim*10) + (10*errG_tv) + (errG_ndvi*7)
        ...
        if epoch%20 == 9:
            torch.save(netG.state_dict(), f'{checkpoint_dir}gensli2_{epoch}.pt')
            torch.save(netD.state_dict(), f'{checkpoint_dir}discsli2_{epoch}.pt')
            torch.save({'optimizerG': optimizerG.state_dict(),
                        'optimizerD': optimizerD.state_dict()},
                        f'{checkpoint_dir}optimizerli2_.pt')
        schedulerG.step()
        schedulerD.step()
...
if __name__ == '__main__':
    ...
    if args.do_log:
        ...
        run = wandb.init(
            ...
            project="ESRGAN_sentinel_Lglean",
            # resume=True,
            name=args.name,
    ...
    save_dir = "../srgan_samples_sent_glean_light/"
    ...
    netG.generator.load_state_dict(torch.load('./stylegan/stylegan_latest.pt', map_location=torch.device(device)), strict=False)
    ...
    niqe = NIQE(s.ratio, './model/niqe_model.mat').to(device)
